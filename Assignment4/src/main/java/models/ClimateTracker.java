...
public class ClimateTracker {
    ...
    public Map<Station, Integer> numberOfMeasurementsByStation() {
        //Nina
        return this.getStations().stream().collect(Collectors.toMap(
                s -> s,
                s -> s.getMeasurements().size()
        ));
    }
    ...
    public Map<Station, LocalDate> firstDayOfMeasurementByStation() {
        ...
        return this.getStations().stream()
                .filter(check -> check.firstDayOfMeasurement().isPresent())
    ...
    public Map<Station, Integer> numberOfValidValuesByStation(Function<Measurement, Double> mapper) {
        ...
        return getStations().stream().collect(Collectors.toMap(
                s -> s,
                s -> s.numValidValues(mapper)
        ));
    }
    ...
    public Map<Integer, Double> annualAverageTemperatureTrend() {
        ...
        Map<Integer, Double> returnMap = getStations().stream()
                .flatMap(station -> station.getMeasurements().stream())
                .filter(x -> !Double.isNaN(x.getAverageTemperature()))
                .collect(Collectors.groupingBy(Measurement::getYear,
                        Collectors.averagingDouble(Measurement::getAverageTemperature)));

        return new TreeMap<>(returnMap);
    }
    ...
    public Map<Integer, Double> annualMaximumTrend(Function<Measurement, Double> mapper) {
        ...
        Map<Integer, DoubleSummaryStatistics> dailyQuantity = getStations().stream()
                .flatMap(station -> station.getMeasurements().stream())
                .filter(trend -> !Double.isNaN(mapper.apply(trend)))
                .collect(Collectors.groupingBy(Measurement::getYear, Collectors.summarizingDouble(mapper::apply)));

        Map<Integer, Double> returnMap = new TreeMap<>();

        dailyQuantity.forEach((key, value) -> returnMap.put(key, value.getMax()));
    ...
    public Map<Month, Double> allTimeAverageDailySolarByMonth() {
        ...
        Map<Month, Double> returnMap = getStations().stream()
                .flatMap(station -> station.getMeasurements().stream())
                .filter(x -> !Double.isNaN(x.getSolarHours()))
                .collect(Collectors.groupingBy(Measurement::getMonth,
    ...
    public int coldestYear() {
        Map<Object, Double> coldestYear = this.getStations().stream()
                // filter the values that are greater than zero
                .flatMap(station -> station.getMeasurements()
                        .stream().filter(measurement -> measurement.getMinTemperature() < 0))
                // collect the output and group the key as year by the values with the minimumtemperatures of the year
                .collect(Collectors.groupingBy(measurement -> measurement.getDate().getYear(),
                        TreeMap::new, Collectors.summingDouble(Measurement::getMinTemperature)));

        return (int) Collections.min(coldestYear.entrySet(), Map.Entry.comparingByValue()).getKey();
    }
