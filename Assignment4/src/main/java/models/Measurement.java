...
public class Measurement {
    ...
    public static Measurement fromLine(String textLine, Map<Integer, Station> stations) {
        String[] fields = textLine.split(",\s*");
        ...
        double[] doubleFields = Arrays.stream(fields)
                .map(String::trim)
                .mapToDouble(fieldString -> fieldString.isEmpty() ? Double.NaN : Double.parseDouble(fieldString))
                .map(field -> field * 0.1)
                .toArray();

        m.averageTemperature = Measurement.check(doubleFields[FIELD_TG], false);
        m.averageWindSpeed = Measurement.check(doubleFields[FIELD_FG], false);
        m.maxHourlyPrecipitation = Measurement.check(doubleFields[FIELD_RHX], true);
        m.maxTemperature = Measurement.check(doubleFields[FIELD_TX], false);
        m.minTemperature = Measurement.check(doubleFields[FIELD_TN], false);
        m.precipitation = Measurement.check(doubleFields[FIELD_RH], true);
        m.solarHours = Measurement.check(doubleFields[FIELD_SQ], true);
        m.maxWindGust = Measurement.check(doubleFields[FIELD_FXX], false);

        return m;
    }

    private static double check(double checkingTrim, boolean shouldCheckNegative) {
        double doubleValue = checkingTrim;
        if (shouldCheckNegative) {
            if (doubleValue == -1) {
                doubleValue = 0.0;
            }
        }
